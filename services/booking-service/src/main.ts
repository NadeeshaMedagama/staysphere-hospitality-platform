...
async function bootstrap(): Promise<void> {
  ...
  // /health, /ready and /metrics stay unversioned so probes and the Prometheus
  // scrape config never have to change when the API version bumps.
  ...
  if (env.NODE_ENV !== 'production') {
    const document = SwaggerModule.createDocument(
      app,
      new DocumentBuilder()
        .setTitle('StaySphere — Booking Service')
        .setDescription('Availability, quoting, reservations and cancellations.')
