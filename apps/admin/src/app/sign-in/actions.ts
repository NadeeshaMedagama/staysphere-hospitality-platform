...
export async function signInAction(
  ...
  if (!staff) {
    return { error: 'This account does not have access to the operations console.', email };
  }

  await adminSession.setSession(result.data);
...
export async function signOutAction(): Promise<void> {
  const session = await adminSession.clearSession();
